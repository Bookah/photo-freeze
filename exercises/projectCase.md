...
2)	Overview page
 ...
 This page displays all uploaded images. It should get all images and handle the actual POST call to save a new guess in the database. 

3) Image card

 This element is not a full page, but is used on the overview page. For each image, you can guess a word and display a hint. All guesses are aggregated over users to display the total number of correct and incorrect attempts for each image.
...
### Git repo
Clone the Github repository from ?????. Add a folder in src with your name. You will create your components in this folder. Make sure that the project runs by running `polymer serve --open` in the command line.

## Hands on: upload page
### Step 1: Design the page
- As a team, create a (mainly visual) overview for the page. You could consider to use the following existing components: paper-input, paper-button. See [webcomponents.org](http://www.webcomponents.org) for more information.
### Step 2: Build your page in the existing application
- Create a new page and link in the application. Click [here](https://www.polymer-project.org/2.0/start/toolbox/create-a-page) for more information.
...
- Find a way to upload a photo in your browser and implement this in JS function(s) (you can use file input. Follow [this]???????? link for more information).
- Make sure the image/photo format is a base64 string.
### Step 5: Save the data in a database
...
### Step 1: Build your page in the existing application
- Create a new page and link in the application.  Click [here](https://www.polymer-project.org/2.0/start/toolbox/create-a-page) for more information.
### Step 2: Display photos/images in database
...
### Step 4: Implement my-photo-card
...
### Step 5: Update the image information
...
- Create a new custom element (e.g. my-photo-card / my-photo-display). You will create functionality to show an image and guesses up to now. Also, it should allow users to input text and view a hint.
- For now, add some text and display it on an existing page so you can check that you created the element correctly.
### Step 3: Create a container
- Create three properties on your element: 1) image, 2) hint, 3) attempts, 4) guess, 5) photoId.
